...
const getDatastoreId = (inputDatastoreId) => {
    const id = inputDatastoreId || process.env.VERTEX_AI_DATASTORE_ID;
    if (!id) {
        throw new Error('Vertex AI Datastore ID is missing. Please set the VERTEX_AI_DATASTORE_ID environment variable or pass it in the request.');
    }
    return id;
};
